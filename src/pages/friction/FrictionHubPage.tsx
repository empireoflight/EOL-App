...
function statusCta(status: SessionStatus): string {
  if (status === 'closed') return 'Read outcome'
  if (status === 'guide_ready' || status === 'scheduled' || status === 'discussed') return 'Continue'
  return 'Add yours'
}
...
function FrictionThreadRow({ teamId, session, isLast }: { teamId: string | undefined; session: ConvergenceSession; isLast: boolean }) {
  ...
  return (
    <Link
      to={`/teams/${teamId}/friction/sessions/${session.id}`}
      className="flex gap-4 py-5"
      style={{ borderBottom: isLast ? 'none' : '1px solid var(--color-eol-border)' }}
    >
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="m-0 mt-0.5 text-[12.5px]" style={{ color: 'var(--color-eol-text-secondary)' }}>
              No tracking, no streaks. Open one on your own, or put it on screen in a meeting.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {moments.map((m) => (
            ...
            >
              <div className="text-[10.5px] font-semibold uppercase tracking-wide" style={{ color: 'var(--color-eol-accent-label)' }}>
                {m.meta}
              </div>
              <div className="text-[15px] font-semibold leading-snug" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-eol-text)' }}>
                {m.name}
              </div>
              <div className="text-[12px] leading-relaxed" style={{ color: 'var(--color-eol-text-secondary)' }}>
                {m.blurb}
              </div>
            </Link>
          ))}
